...
function isForbiddenAngle(angle: Angle) {
  ...
  return [
    "raises questions", "highlights the need", "could exacerbate", "may exacerbate",
    "winner-takes-all", "future of work", "responsible innovation", "need to adapt",
    "need to upskill", "need to reskill", "commitment to innovation", "set a precedent",
...
function selectSafeAngles(angles: Angle[]) {
  ...
  for (const angle of angles) {
    ...
    const key = angle.angle.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
...
{"evidence":[{"claim":"short concrete point from the supplied story","support":"headline or summary detail","type":"fact"}],"angles":[{"angle":"specific thesis","why":"why this is interesting","evidence":"the story detail that supports this angle"}]}`;
  const parsed = parseJson(await provider().generateText(prompt, { format: "json", temperature: 0.2, numPredict: 900 }));
  return { evidence: parseEvidence(parsed?.evidence), angles: selectSafeAngles(parseAngles(parsed?.angles)) };
}

export async function generateEditorialAngles(story: Story) {
  const startedAt = Date.now();
  const editorial = await buildEditorialPass(story);
  console.info(`[PostCraft] editorial_ms=${Date.now() - startedAt} model_only=true evidence=${editorial.evidence.length} angles=${editorial.angles.length}`);
  if (!editorial.angles.length) throw new Error("PostCraft could not find a useful angle in this story. Try another story.");
  return { angles: editorial.angles, evidence: editorial.evidence };
}

function validateEvidence(value: unknown): Evidence[] { return parseEvidence(value); }

function postHasConcreteAnchor(post: string) {
  return /\b\d+(?:\.\d+)?%|\$\d|\d+(?:\.\d+)?\s*(?:trillion|billion|million|x)\b|scenario|model|share|wage|unemployment|entry-level|research|proof|jobs|workers/i.test(post);
}

function postHasGenericFiller(post: string) {
  return ["it's crucial to recognize", "not evenly distributed", "highlights the need", "raises a crucial question", "strike a balance", "in today's rapidly changing world", "the future of work", "what do you think", "agree or disagree"].some((phrase) => post.toLowerCase().includes(phrase));
}
...
export async function generateEditorialPost(story: Story, angle: string, angleWhy: string, modeInstruction: string, suppliedEvidence?: Evidence[]) {
  ...
  if (evidence.length < 1) evidence = (await buildEditorialPass(story)).evidence;
