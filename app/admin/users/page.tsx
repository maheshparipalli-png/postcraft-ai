      ...
      <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8">
        ...
        <div className="mt-7 grid gap-3 sm:grid-cols-[1fr_180px_160px]">
          <input
            ...
            className="border border-neutral-300 bg-white/70 px-4 py-3 text-sm outline-none focus:border-neutral-700"
          />
          <select value={status} onChange={(e) => setStatus(e.target.value)} className="border border-neutral-300 bg-white/70 px-3 py-3 text-sm">
          ...
          <select value={role} onChange={(e) => setRole(e.target.value)} className="border border-neutral-300 bg-white/70 px-3 py-3 text-sm">
        ...
        <div className="mt-7 overflow-x-auto border-y border-neutral-300">
          <table className="w-full min-w-[1000px] text-left text-sm">
            <thead className="border-b border-neutral-300 text-[10px] uppercase tracking-[0.16em] text-neutral-500">
              <tr>
                <th className="px-3 py-4">User</th><th className="px-3 py-4">Role</th><th className="px-3 py-4">Status</th><th className="px-3 py-4">Trial ends</th><th className="px-3 py-4">Resets</th><th className="px-3 py-4"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-neutral-300">
              {filtered.map((user) => (
                <tr key={user.user_id} className="hover:bg-white/50">
                  <td className="px-3 py-5">
                    <div className="font-medium">{user.display_name || "Unnamed user"}</div>
                    <div className="mt-1 text-xs text-neutral-500">{user.email || "No email"}</div>
                    <div className="mt-1 font-mono text-[10px] text-neutral-400">{user.user_id}</div>
                  </td>
                  <td className="px-3 py-5 text-xs">{user.role}</td>
                  <td className="px-3 py-5"><span className="rounded-full border border-neutral-300 px-2 py-1 text-[11px]">{user.status}</span></td>
                  <td className="px-3 py-5 text-xs">{formatDate(user.trial_ends_at)}</td>
                  <td className="px-3 py-5 text-xs">{user.trial_reset_count}</td>
                  <td className="px-3 py-5 text-right"><Link href={`/admin/users/${user.user_id}`} className="text-xs font-medium underline underline-offset-4">Manage →</Link></td>
                </tr>
              ))}
