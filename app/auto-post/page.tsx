...
export default function AutoPostPage() {
    ...
    (async () => {
      try {
        ...
        if (preferencesResponse.ok && !preferences?.completed) {
          window.location.assign("/interests?next=/auto-post");
          return;
        }
  ...
  async function prepareDraft(force = false) {
    ...
    setVisualUrl("");
    setPublishStatus("");
    if (force) setRegenerationFailed(false);
    try {
  ...
  return (
    <main className="min-h-screen bg-[#f4f3ef] text-[#171717]">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <footer className="flex flex-wrap items-center justify-between gap-4 border-t border-neutral-300/80 py-8 text-xs text-neutral-500"><span>PostCraft AI · Review before you publish.</span><Link href="/interests?setup=1" className="underline underline-offset-4">Change interests</Link></footer>
      </div>
    </main>
  );
}
