...
export default function AutoPostPage() {
  ...
  async function prepareDraft(force = false) {
    ...
    setRunning(true); setError(""); setVisualUrl(""); setPublishStatus("");
    try {
  ...
  return <main className="min-h-screen bg-[#f4f3ef] text-[#171717]"><div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
    ...
    <section className="border-b border-neutral-300/80 py-12 lg:py-16"><div className="mb-4 text-[11px] font-medium uppercase tracking-[0.2em] text-neutral-500">Create · AI-assisted editorial workflow</div><h1 className="max-w-4xl font-serif text-5xl leading-[0.98] tracking-[-0.05em] sm:text-7xl">Your next useful post,<br />prepared for you.</h1><p className="mt-6 max-w-2xl text-base leading-7 text-neutral-600">Discover a verified AI story, choose a useful perspective, and review a LinkedIn-ready draft before publishing.</p><div className="mt-8 flex flex-wrap gap-3"><button type="button" onClick={() => void prepareDraft()} disabled={running || (!!preview && !published)} className="bg-neutral-900 px-5 py-3 text-sm font-medium text-white disabled:cursor-not-allowed disabled:bg-neutral-400">{running ? "Preparing draft…" : preview && !published ? "Draft ready" : "Prepare today’s post →"}</button>{preview && !published && <button type="button" onClick={() => void regenerateDraft()} disabled={running} className="border border-neutral-300 bg-white/50 px-5 py-3 text-sm disabled:opacity-50">Regenerate draft</button>}<Link href="/create" className="border border-neutral-300 bg-white/50 px-5 py-3 text-sm">Write manually</Link></div></section>
    <section className="grid gap-8 border-b border-neutral-300/80 py-10 lg:grid-cols-[280px_1fr]"><aside className="space-y-6"><div><div className="text-[10px] uppercase tracking-[0.18em] text-neutral-500">How it works</div><p className="mt-3 text-sm leading-6 text-neutral-600">Auto-post prepares one draft for your review. You can regenerate an unsent draft, then choose text, visual, or one combined LinkedIn post containing both.</p></div><div className="border-t border-neutral-300 pt-5 text-xs leading-6 text-neutral-500"><strong className="text-neutral-900">Next step:</strong><br /><Link href="/auto-publish" className="underline underline-offset-4">Configure daily automation →</Link></div></aside>
      <div className="min-w-0"><div className="flex items-end justify-between gap-4 border-b border-neutral-300 pb-5"><div><div className="text-[10px] uppercase tracking-[0.18em] text-neutral-500">Today’s recommendation</div><h2 className="mt-2 font-serif text-3xl tracking-[-0.03em] sm:text-4xl">One story. One clear point.</h2></div>{preview && <span className={`rounded-full px-3 py-1 text-[10px] font-medium ${published ? "bg-blue-100 text-blue-800" : "bg-amber-100 text-amber-800"}`}>{published ? "Published" : "Ready for review"}</span>}</div>
        ...
        {preview && <div className="mt-6 space-y-6"><div className="flex flex-wrap items-center justify-between gap-3 border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><span><strong>{published ? "Published." : "Draft ready for review."}</strong> {published ? "This post has already been published." : "You can regenerate it before publishing."}</span>{!published && <button type="button" onClick={() => void regenerateDraft()} disabled={running} className="font-medium underline underline-offset-4 disabled:opacity-50">Regenerate</button>}</div><article className="border border-neutral-300 bg-white/70 p-6 sm:p-8"><div className="flex flex-wrap gap-2 text-[10px] uppercase tracking-[0.16em] text-neutral-500"><span className="rounded-full bg-emerald-100 px-2 py-1 text-emerald-800">Verified source</span><span>{preview.article.source}</span><span>·</span><span>{preview.article.publishedAt || "Date unavailable"}</span></div><h3 className="mt-5 max-w-3xl font-serif text-3xl leading-tight sm:text-4xl">{preview.article.title}</h3>{preview.ranking?.reason && <p className="mt-4 max-w-2xl text-sm leading-6 text-neutral-600">{preview.ranking.reason}</p>}<a href={preview.article.url} target="_blank" rel="noreferrer" className="mt-5 inline-block text-xs font-medium underline underline-offset-4">Read original source →</a></article>
          <div className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]"><div className="border border-neutral-300 bg-white/50 p-6"><div className="text-[10px] uppercase tracking-[0.18em] text-neutral-500">Recommended angle</div><p className="mt-4 text-lg leading-7">{preview.angle?.angle}</p>{preview.angle?.why && <p className="mt-4 text-sm leading-6 text-neutral-500">{preview.angle.why}</p>}</div>
            <div className="border border-neutral-300 bg-[#171717] p-6 text-white sm:p-8"><div className="flex items-center justify-between gap-3"><div className="text-[10px] uppercase tracking-[0.18em] text-neutral-400">LinkedIn post</div><span className="text-[10px] text-neutral-400">{published ? "Published" : "Ready for review"}</span></div><div className="mt-5 flex flex-wrap gap-2"><button type="button" onClick={() => void selectFormat("combined")} disabled={published || rendering} className={`px-3 py-2 text-xs ${format === "combined" ? "bg-white text-neutral-900" : "border border-white/30 text-white"}`}>Text + visual</button><button type="button" onClick={() => void selectFormat("text")} disabled={published} className={`px-3 py-2 text-xs ${format === "text" ? "bg-white text-neutral-900" : "border border-white/30 text-white"}`}>Text only</button><button type="button" onClick={() => void selectFormat("image")} disabled={published || rendering} className={`px-3 py-2 text-xs ${format === "image" ? "bg-white text-neutral-900" : "border border-white/30 text-white"}`}>{rendering ? "Rendering…" : "Visual only"}</button></div>{(format === "combined" || format === "image") && visualUrl ? <img src={visualUrl} alt="Preview of the LinkedIn visual post" className="mt-6 max-h-[560px] w-full object-contain" /> : <pre className="mt-6 whitespace-pre-wrap font-serif text-lg leading-8 text-neutral-100 sm:text-xl">{preview.post}</pre>}{format === "combined" && <p className="mt-5 border-t border-white/15 pt-5 text-xs leading-5 text-neutral-400">LinkedIn will publish the visual first, with the full text caption beneath it, as one post.</p>}<div className="mt-7 flex flex-wrap items-center gap-4 border-t border-white/15 pt-5"><button type="button" onClick={copyPost} disabled={published} className="bg-white px-4 py-2.5 text-xs font-medium text-neutral-900 disabled:opacity-50">Copy caption</button><button type="button" onClick={publishPost} disabled={published || publishStatus === "Publishing…" || rendering} className="bg-emerald-500 px-4 py-2.5 text-xs font-medium text-white disabled:opacity-50">{publishStatus || `Publish ${format === "combined" ? "text + visual" : format === "image" ? "visual" : "text"} to LinkedIn`}</button>{copyStatus && <span className="text-xs text-emerald-300">{copyStatus}</span>}</div></div></div></div>}
      </div></section>
    </div></main>;
}
