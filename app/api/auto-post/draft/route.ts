...
function billingError(status: string) {
  return NextResponse.json({ error: status === "expired" ? "Your free trial has expired. Subscribe to continue." : "Start your free trial or subscribe to continue.", status }, { status: 402 });
}

export async function GET() {
  try {
    const billing = await getBillingAccess();
    if (!billing.authenticated) return NextResponse.json({ error: "Please sign in first." }, { status: 401 });
    if (!billing.allowed) return billingError(billing.status);
