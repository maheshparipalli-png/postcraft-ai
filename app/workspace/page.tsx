...
export default function Workspace() {
  ...
  async function selectIdea(value: Idea) {
    ...
    try {
      const prompt = `You are PostCraft AI, an editorial thinking partner. Analyze this exact news story and its source article. Find three distinct, evidence-led LinkedIn angles. Do not rely on outside knowledge. Return ONLY a JSON array with objects containing angle, why, and evidence.\n\nHeadline: ${value.title}\nSource: ${value.source}\nURL: ${value.url}\nSummary: ${value.description || "No reliable summary was supplied."}`;
  ...
  async function createPost() {
    ...
    const prompt = `You are PostCraft AI. Write a LinkedIn post from a specific point of view, grounded only in the supplied evidence.\n\nStory: ${idea.title}\nSource: ${idea.source}\nURL: ${idea.url}\nSelected angle: ${angle}\nWhy the angle works: ${angles.find((item) => item.text === angle)?.why || ""}\nPerspective: ${selectedPerspective?.label}\nPerspective guidance: ${selectedPerspective?.description}\nUser's own note: ${perspectiveNote || "No additional note supplied."}\nEvidence JSON: ${JSON.stringify(evidence)}\n\nRules: The user's perspective is the point of view, not a fact. Do not invent personal experience, examples, statistics, motives, or conclusions not supported by the evidence. Keep the selected angle intact. Use plain language. Make the thesis clear early. Avoid generic phrases such as "raises important questions", "future of work", "need to strike a balance", or "in today's rapidly changing world". Do not add a generic policy conclusion. Return ONLY JSON: {"post":"..."}`;
    try {
    ...
    <main className="min-h-screen bg-white text-neutral-900">
      <div className="mx-auto max-w-5xl px-6 py-10">
        <header className="flex items-center justify-between">
          ...
          <a href="/" className="text-sm text-neutral-500 underline underline-offset-4">Back to Discover</a>
        </header>
        ...
        {idea && <section className="mt-12 border-t border-neutral-200 pt-10">
          <div className="text-sm font-medium text-neutral-700">1. Choose the argument</div>
          <p className="mt-1 text-sm text-neutral-500">Pick the idea you actually want to defend, challenge, or explore.</p>
          {angleLoading ? <div className="mt-4 rounded-xl border border-neutral-200 bg-neutral-50 p-4 text-sm text-neutral-500">Reading the source and finding grounded angles...</div> : <div className="mt-4 grid gap-3 md:grid-cols-3">{angles.map((item) => <button key={item.text} onClick={() => setAngle(item.text)} className={`rounded-xl border p-4 text-left ${angle === item.text ? "border-neutral-900 bg-neutral-900 text-white" : "border-neutral-200 hover:border-neutral-400"}`}><div className="text-sm font-medium leading-6">{item.text}</div><div className={`mt-3 text-xs leading-5 ${angle === item.text ? "text-neutral-300" : "text-neutral-500"}`}>{item.why}</div>{item.evidence && <div className={`mt-4 border-t pt-3 text-xs ${angle === item.text ? "border-neutral-700 text-neutral-300" : "border-neutral-100 text-neutral-500"}`}><span className="font-semibold uppercase tracking-wide">Evidence</span><div className="mt-1">{item.evidence}</div></div>}</button>)}</div>}

          <div className="mt-10"><div className="text-sm font-medium text-neutral-700">2. What is your take?</div><p className="mt-1 text-sm text-neutral-500">This is the part PostCraft should not invent for you.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{perspectives.map((item) => <button key={item.id} onClick={() => setPerspective(item.id)} className={`rounded-xl border p-4 text-left ${perspective === item.id ? "border-neutral-900 bg-neutral-900 text-white" : "border-neutral-200 hover:border-neutral-400"}`}><div className="text-sm font-medium">{item.label}</div><div className={`mt-1 text-xs ${perspective === item.id ? "text-neutral-300" : "text-neutral-500"}`}>{item.description}</div></button>)}</div><textarea value={perspectiveNote} onChange={(event) => setPerspectiveNote(event.target.value)} placeholder="Optional: add the thought you want PostCraft to preserve..." rows={4} className="mt-4 w-full rounded-xl border border-neutral-200 px-4 py-3 text-sm leading-6 outline-none focus:border-neutral-900" /></div>

          <div className="mt-10"><div className="text-sm font-medium text-neutral-700">3. Create the post</div><p className="mt-1 text-sm text-neutral-500">PostCraft will combine the evidence, the selected argument, and your point of view.</p><button onClick={createPost} disabled={postLoading || angleLoading || !angle} className="mt-4 rounded-lg bg-neutral-900 px-5 py-3 text-sm font-medium text-white disabled:opacity-50">{postLoading ? "Writing..." : "Create LinkedIn post"}</button></div>
        </section>}
