    ...
    <main className="min-h-screen bg-white text-neutral-900">
      <div className="mx-auto max-w-5xl px-6 py-10">
        ...
        {idea && <section className="mt-12 border-t border-neutral-200 pt-10">
          ...
          <p className="mt-1 text-sm text-neutral-500">PostCraft has found three grounded ways into this story. Pick the one you actually want to defend, challenge, or explore.</p>
          ...
          {!angleLoading && angle && <div className="mt-10"><div className="text-sm font-medium text-neutral-700">2. What’s your take?</div><p className="mt-1 text-sm text-neutral-500">PostCraft supplies the argument. You decide what you think about it.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{perspectives.map((item) => <button key={item.id} onClick={() => { setPerspective(item.id); setPost(""); }} className={`rounded-xl border p-4 text-left ${perspective === item.id ? "border-neutral-900 bg-neutral-900 text-white" : "border-neutral-200 hover:border-neutral-400"}`}><div className="text-sm font-medium">{item.label}</div><div className={`mt-1 text-xs ${perspective === item.id ? "text-neutral-300" : "text-neutral-500"}`}>{item.description}</div></button>)}</div><textarea value={perspectiveNote} onChange={(event) => { setPerspectiveNote(event.target.value); setPost(""); }} placeholder="Optional: add the thought you want PostCraft to preserve..." rows={4} className="mt-4 w-full rounded-xl border border-neutral-200 px-4 py-3 text-sm leading-6 outline-none focus:border-neutral-900" /></div>}

          {!angleLoading && angle && <div className="mt-10"><div className="text-sm font-medium text-neutral-700">3. Create the post</div><p className="mt-1 text-sm text-neutral-500">PostCraft will combine the evidence, your selected angle, and your point of view.</p><button onClick={createPost} disabled={postLoading || !angle} className="mt-4 rounded-lg bg-neutral-900 px-5 py-3 text-sm font-medium text-white disabled:opacity-50">{postLoading ? "Writing..." : "Create LinkedIn post"}</button></div>}
        </section>}
